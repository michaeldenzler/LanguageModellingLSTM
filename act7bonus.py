    ...
    for i in range(0, block_length, sequence_length):
        ...
        for j in range(batch_size):
            ...
            for k in range(end - start - 1):
                # replace each word by its representing integer and place it into the sequence
...
def main():
    ...
    # Download book from nltk and open it
    nltk.download('gutenberg')
    file = nltk.corpus.gutenberg.words('bible-kjv.txt')

    # Turn book into a string in order to preprocess it
    book = ''
    ...
    # putting the data in a pandas dataframe, each word seperately
    word_df = pd.DataFrame({'words': book})

    # total unique words
    sum = word_df.count()

    # compute frequency of unique word and add it to the data frame
    word_df = word_df['words'].value_counts().to_frame('frequency')
    word_df['frequency'] = word_df['frequency']/word_df['frequency'].sum()
    # represent words with index value based on frequency
    word_df = word_df.reset_index()
    print(word_df)

    # write data frame to file
    f = open('book_dataframe words', 'w+')
    f.write(str(word_df))
    f.close()

    # storing words and replacement values, each pair a separate row
    replacements = []
    for i in range(len(word_df)):
        replacements = np.append(replacements, [word_df.loc[i, 'index'], i])
        word_df.loc[i, 'index'] = i
    replacements = np.reshape(replacements, (-1, 2))
    # turn replacement array into a dictionary with key: words and value: replacement integer
